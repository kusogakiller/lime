 ...
 * The real libavcodec/avcodec.h pulls `codec_par.h` (which declares these)
 * only inside `#ifndef HAVE_AV_CONFIG_H`; our corpus config.h defines
 * HAVE_AV_CONFIG_H, so that include is skipped during Charger's header parse
 * and avcodec_parameters_alloc/free/copy never reach the surfaced AST (while
 * avcodec_parameters_from_context/to_context, declared before the struct in
 * codec_par.h, survive). Re-declaring these three exact public prototypes here
 * — in the api_header itself — makes them visible to Charger's parser. `struct
 * AVCodecParameters` is already forward-declared just above, so the pointer
 * types are valid C. These are verbatim FFmpeg public API; the real definitions
 * live in codec_par.c (unchanged). Corpus-config parse-visibility fix only — no
 * ABI change, no library-specific logic in charger.rs. */
